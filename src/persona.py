...
async def run_persona_chat():
    ...
    while True:
        try:
            ...
            if user_msg.lower().startswith("/stage") or user_msg.lower().startswith("/modify"):
...
async def handle_web_slash_command(user_msg: str) -> str:
    ...
    Handles /sandbox, /amend, /repeal, and other slash commands.
    """
    user_msg = user_msg.strip()
    ...
    elif user_msg.lower().startswith("/stage") or user_msg.lower().startswith("/modify"):
        return "[Error] /stage and /modify are disabled. Use the skill staging harness or a Project Sandbox."
